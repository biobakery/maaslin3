...
[**MaAsLin3**](http://huttenhower.sph.harvard.edu/MaAsLin3) is the next
...
MaAsLin 3 is run by specifying a `param_list` that contains the
...
`group(variable_name)` and `ordered(variable_name)` in the formula.
...
and `ordered_effects`. Categorical variables should either be ordered as
...
against 0 (`median_comparison_prevalence`). See below for a discussin of
these paremeters.
...
abundance tools, low prevalence features should not be filtered out**
...
1. Data output files
    ...
    * ``models_LM.rds`` and ``models_logistic.rds``
        * These files contain a list with every model fit object (`LM` for
...
```{r, echo=FALSE,out.width="49%",out.height="20%",
    fig.cap="The four types of association plots",
    fig.show='hold',fig.align='center', cache = FALSE, include=FALSE}
prefix <- "hmp2_output/figures/association_plots"
plot_vec <- c("/age/LM/age_Faecalibacterium_prausnitzii_LM.png",
            "/dysbiosis_state/LM/dysbiosis_state_Escherichia_coli_LM.png",
...
can be run with the same `param_list` as the original `maaslin3` run to
...
Likewise, `heatmap_vars` and `coef_plot_vars` can be specified in the
`param_list` at any point, but it is usually easier to see how the names
...
```{r, echo=FALSE,out.width="49%",out.height="20%",
    fig.cap="The four types of association plots",fig.show='hold',
    fig.align='center', cache = FALSE, include=FALSE}
prefix <- "hmp2_output/figures/association_plots"
plot_vec <- c("/age/LM/Age_Faecalibacterium prausnitzii_LM.png",
            "/Dysbiosis/LM/Dysbiosis_Escherichia coli_LM.png",
...
correlated data problem. In particular, **random intercept models may
produce poor model fits when there are fewer than 5 observations per
group.** In these scenarios, per-group fixed effects should be used. For
...
should be included as fixed effects, but only the pre/post indicator
should be retained for analysis afterwards.

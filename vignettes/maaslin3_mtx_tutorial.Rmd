...
In this first example run on MTX data, we will run MaAsLin 3 on the RNA pathway abundances as characterized by HUMAnN 2 but not normalized by the matched DNA of these samples. When running MTX data, **it is recommended to set `median_comparison_abundance_threshold = 0.0`** in order to detect the typically-smaller MTX effects. (Make sure to set `include=T` to view the plots when knitting the Rmd.)
...
intersect(results_rna$feature, results_rna_dna$feature) # 51 features overlapped

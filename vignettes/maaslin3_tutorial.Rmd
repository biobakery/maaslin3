...
* Total sum scaling (`normalization = 'TSS'`) with a log transformation (`'transform = 'LOG'`) afterwards will be used. These are almost always the recommended choices (and all MaAsLin 3 evaluations were performed with these options), but other normalizations and transformations are allowed (see `normalization` and `transform` in `?maaslin3`). 
...
```{R, echo = T, results = 'hide', warning = FALSE, cache = F}
...
# Write results
write.table(all_results, 'hmp2_output/all_results.tsv', sep='\t', row.names = F)
...
```{r, engine = 'bash', eval = FALSE, cache = F}

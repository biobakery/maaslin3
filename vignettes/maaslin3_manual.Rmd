...
and group predictors can be included by including `group(variable_name)`
and `ordered(variable_name)` in the formula. Ordered and group
...
A `SummarizedExperiment` object can also be provided as input:

```{r}
# Create example SE
se <- SummarizedExperiment::SummarizedExperiment(assays = t(taxa_table), 
                                                colData = metadata)
...
is recommended, but `PLOG` (pseudo-log) and `NONE` can also be used.
...
1. In linear models, if two features' associations with a particular
...
`heatmap_vars` in `param_list` with the desired variables.

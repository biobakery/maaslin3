...
* [3. Running the MTX model](#3-running-the-mtx-model)
    ...
    * [3.3 Running models in R](#33-running-models-in-R)
    	* [3.3.1 Raw RNA Abundances with MaAsLin 3](#331-raw-rna-abundances-with-maaslin-3)
    	* [3.3.2 RNA/DNA Ratios with MaAsLin 3](#332-rna-dna-ratios-with-maaslin-3)
    	* [3.3.3 RNA abundance adjusted by DNA abundance with MaAsLin 3](#333-rna-abundance-adjusted-by-dna-abundance-with-mtxmodel)
    * [3.4 Compare output](#34-compare-output)

## 1. Description {#1-description}

In this tutorial, we will walk through most of the steps from [the MTX model manuscript](https://academic.oup.com/bioinformatics/article/37/Supplement_1/i34/6319701). We will compare the output of unadjusted MaAsLin 3 runs on MTX data with ratio-adjusted MTX data using DNA copy number and MTX abundance data adjusted with DNA abundances. 

<img src="https://github.com/biobakery/biobakery/blob/master/images/Figure_MTXmodel.png" width="70%" />
...
The MTX model in MaAsLin 3 can be run from the command line or as an R function. Both methods require the same arguments, have the same options, and use the same default settings.
...
Running the MTX model in MaAsLin 3 generates the same output files as usual. See more details in the [MaAsLin 3 manual](https://github.com/biobakery/Maaslin3?tab=readme-ov-file#output-files).
...
In this first example run on MTX data, we will run MaAsLin 3 on the RNA pathway abundances as characterized by HUMAnN 2 but not normalized by the matched DNA of these samples. When running MTX data, **it is recommended to set `median_comparison_abundance_threshold = 0.0`** in order to detect the typically-smaller MTX effects.

```{R, echo = T, results = 'hide', warning = FALSE, cache = T}
# Set median_comparison_abundance_threshold = 0.0 since effect sizes are small
param_list <- list(input_data = df_input_data, 
                   ...
                   heatmap_vars = c('dysbiosis_state dysbiosis_UC', 'diagnosis UC', 'age', 'antibiotics Yes'))
...
### 3.3.2 RNA/DNA Ratios with MaAsLin 3 {#332-rna-dna-ratios-with-maaslin-3}
Next, we will run the same model changing the input to the RNA/DNA ratio data frame. Note that we now set `normalization = 'NONE'` so that MaAsLin 3 does not total-sum scale the ratios. As before, we set `median_comparison_abundance_threshold = 0.0` in order to detect the typically-smaller MTX effects.

```{R, echo = T, results = 'hide', warning = FALSE, cache = T}
# Set median_comparison_abundance_threshold = 0.0 since effect sizes are small
param_list <- list(input_data = df_input_dataratio, 
                   input_metadata = df_input_metadata, 
...
Finally, let's use some simple R scripts to compare the results from each model. First, we will look at the number of significant [dysbiosis](https://www.nature.com/articles/s41586-019-1237-9) associations. To do this we will use the base R function ``subset`` to subset the results to just the ones from the dysbiosis comparisons and ``table`` to count the number of pathways that were associated with UC/CD in each model. 
...
intersect(results_rna_ratio$feature, results_rna$feature) # 9 features overlapped

# features called by the Raw RNA abundances compared to the RNA abundances adjusted by the DNA abundances 
intersect(results_rna$feature, results_rna_dna$feature) # 47 features overlapped

# features called by the RNA/DNA ratios compared to the RNA abundances adjusted by the DNA abundances 
intersect(results_rna_ratio$feature, results_rna_dna$feature) # 10 features overlapped
```
...
```
Here, you can tell that model choice influences the effect size, particularly for the prevalence models.

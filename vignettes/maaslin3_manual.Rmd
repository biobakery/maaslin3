...
formulas <- maaslin_check_formula(data,
                                  metadata,
                                  '~ diagnosis + dysbiosis_state + antibiotics + age + reads')
...
In the new summary plot below, we can see that the feature names are cleaned up, the metadata names are cleaned up, the set of metadata variables used in the coefficient plot is different, and the metadata used in the heatmap is reordered. (Make sure to set `include=T` to view the plots when knitting the Rmd.)

...
``HMP2_metadata.tsv``: is a tab-delimited file with samples as rows and metadata as columns. It is a subset of the metadata file that just includes some of the fields.
...
The MTX model generates the same types of output files with MaAsLin 3: data and visualization. See more details in the [MaAsLin 3 manual](https://github.com/biobakery/Maaslin3?tab=readme-ov-file#output-files). For this tutorial, we also will run two models using `maaslin3` instead of the wrapper. 
